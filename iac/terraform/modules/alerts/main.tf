...
module "security_group_changes" {
  ...
  alarm_actions                   = ["${module.sns_topic.sns_topic_arn}", "arn:${data.aws_partition.current.partition}:lambda:${var.region}:${data.aws_caller_identity.current.id}:function:${var.lambda_function_name}"]
